            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="container">
                                            ...
                                            <div class="flex no-gutters flex-wrap">
                                                <div class="col-md-4 border-r border-gray-200">
                                                    <div class="ratings text-center p-4 py-5"> <span
                                                            class="badge bg-success">User</span> <span class="d-block">
                                                            <h1 class="text-xl">{{ $cn->name }}
                                                        </span> </div>
                                                </div>
                                                <div class="col-md-8 text-left pl-5 mt-auto mb-2">
                                                    <p>{{ $cn->comment }}</p>
                                                    <div class="flex justify-end align-end mr-2 flex-wrap">
                                                        <button type="button" class="m-2  btn btn-dark"><a
                                                                style="color:white"
                                                                href="/newsarticles/{{ $news2->id }}/comments/{{ $cn->id }}/edit">
                                                                Edit </a></button>

                                                        <form
                                                            action="/newsarticles/{{ $news2->id }}/comments/{{ $cn->id }}"
                                                            method="post">
                                                            @method('DELETE')
                                                            @csrf

                                                            <button type="submit"
                                                                class="m-2 btn btn-danger">Delete</button>
                                                        </form>
                                                        @if (!$cn->is_approved)
                                                            <button type="button" class="m-2 btn btn-success"><a
                                                                    style="color:white"
                                                                    href="/newsarticles/{{ $news2->id }}/comments/{{ $cn->id }}/approve">
                                                                    Approve
                                                                </a></button>
                                                        @elseif (! $cn->is_hidden)
                                                            <button type="button" class="m-2 btn btn-warning"><a
                                                                    style="color:white"
                                                                    href="/newsarticles/{{ $news2->id }}/comments/{{ $cn->id }}/hide">
                                                                    Hide
                                                                </a></button>
                                                        @else
                                                            <button type="button" class="m-2 btn btn-success"><a
                                                                    style="color:white"
                                                                    href="/newsarticles/{{ $news2->id }}/comments/{{ $cn->id }}/appear">
                                                                    Show
                                                                </a></button>
                                                        @endif
                                                    </div>
                        ...
                        <form action="/newsarticles/{{ $news2->id }}/comments" method="POST"
